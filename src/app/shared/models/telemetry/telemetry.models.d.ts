...
export declare enum WsCmdType {
    ...
    RECIPE_UNSUBSCRIBE = "RECIPE_UNSUBSCRIBE"
}
...
/** Subscribe to recipe / recipe_data lifecycle events (Recipe Manager
 *  widget). `recipeIds` empty = all recipes the user can see. */
export declare class RecipeSubCmd implements WebsocketCmd {
    cmdId: number;
    recipeIds: string[];
    type: WsCmdType;
    constructor(recipeIds?: string[]);
}
export declare class RecipeUnsubscribeCmd implements WebsocketCmd {
    cmdId: number;
    type: WsCmdType;
}
...
/** Recipe lifecycle event push — id-only; the widget refetches on receipt. */
export interface RecipeUpdateMsg extends CmdUpdateMsg {
    cmdUpdateType: CmdUpdateType.RECIPE;
    eventType: string;
    recipeId: string;
    recipeDataId?: string;
}
export type WebsocketDataMsg = AlarmDataUpdateMsg | AlarmCountUpdateMsg | EntityDataUpdateMsg | EntityCountUpdateMsg | SubscriptionUpdateMsg | NotificationCountUpdateMsg | NotificationsUpdateMsg | RecipeUpdateMsg;
...
export declare const isRecipeUpdateMsg: (message: WebsocketDataMsg) => message is RecipeUpdateMsg;
...
/** Subscriber for recipe / recipe_data lifecycle events (Recipe Manager
 *  widget). Rides the shared telemetry websocket the same way
 *  NotificationSubscriber does. `recipeEvents$` emits an id-only event;
 *  the widget refetches in response. */
export declare class RecipeSubscriber extends WsSubscriber {
    private websocketService;
    protected zone?: NgZone;
    private recipeEventSubject;
    recipeEvents$: Observable<RecipeUpdateMsg>;
    static create(websocketService: WsService<any>, zone: NgZone, recipeIds?: string[]): RecipeSubscriber;
    constructor(websocketService: WsService<any>, zone?: NgZone);
    onRecipeUpdate(message: RecipeUpdateMsg): void;
    complete(): void;
}
