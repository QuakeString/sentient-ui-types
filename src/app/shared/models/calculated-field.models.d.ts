...
interface BaseCalculatedField extends Omit<BaseData<CalculatedFieldId>, 'label'>, HasVersion, HasEntityDebugSettings, HasTenantId, ExportableEntity<CalculatedFieldId> {
    entityId: EntityId;
    additionalInfo?: any;
    /** Enable/disable state. Backend defaults absent to enabled. */
    enabled?: boolean;
}
...
export declare const calculatedFieldTypes: (CalculatedFieldType.SIMPLE | CalculatedFieldType.SCRIPT | CalculatedFieldType.GEOFENCING | CalculatedFieldType.PROPAGATION | CalculatedFieldType.RELATED_ENTITIES_AGGREGATION | CalculatedFieldType.ENTITY_AGGREGATION | CalculatedFieldType.EVENT)[];
/**
 * EVENT type: the batch/event definition executed by the analytics
 * materialiser. `event.config` is the operator config the backend runs;
 * `event.uiConfig` preserves the form's own shape so editing round-trips.
 */
export interface CalculatedFieldEventConfiguration {
    type: CalculatedFieldType.EVENT;
    arguments: Record<string, unknown>;
    /** Unused for EVENT; present so the configuration union stays uniform. */
