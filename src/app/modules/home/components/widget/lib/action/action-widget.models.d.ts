...
export declare abstract class BasicActionWidgetComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    /**
     * Whether the current user may actuate the device. Centralized RBAC gate for
     * ALL control widgets: every write goes through {@link updateValue}, which is
     * blocked at runtime when this is false. Derived from the platform
     * `DEVICE:WRITE` generic permission and kept live via `permissionsChanged$`.
     */
    protected canWrite: boolean;
    private permissionService;
    private permissionSub;
    loading$: Observable<boolean>;
    protected constructor(cd: ChangeDetectorRef);
    ngOnInit(): void;
    /** Recompute device-write permission. */
    protected applyWritePermission(): void;
    /** Hook for subclasses to react when the user's write permission changes at runtime. */
...
export declare class DoNothingValueSetter<V> extends ValueSetter<V> {
    protected ctx: WidgetContext;
    protected settings: SetValueSettings;
    protected simulated: boolean;
    constructor(ctx: WidgetContext, settings: SetValueSettings, simulated: boolean);
    setValue(_value: V): Observable<any>;
    protected doSetValue(_data: any): Observable<any>;
}
