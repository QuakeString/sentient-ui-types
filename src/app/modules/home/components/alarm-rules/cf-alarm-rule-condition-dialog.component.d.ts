...
import { AlarmRuleCondition, AlarmRuleConditionType, AlarmRuleExpressionType, AlarmRuleFilter } from '@shared/models/alarm-rule.models';
...
/**
 * Ported from TB CE
 * (~/thingsboard/.../alarm-rules/cf-alarm-rule-condition-dialog.component.ts).
 *
 * AR2c scope: SIMPLE filter expression only — the filter list inside
 * the SIMPLE branch is a placeholder until AR2d wires the predicate
 * builder. RHAI/Script (TBEL) mode is laid out and bindable, but the
 * test-script button stays inert until AR2f. DURATION/REPEATING
 * static + dynamic value forms are fully ported.
 */
export declare class CfAlarmRuleConditionDialogComponent extends DialogComponent<CfAlarmRuleConditionDialogComponent, AlarmRuleCondition> {
    ...
    conditionFormGroup: import("@angular/forms").FormGroup<{
        expression: import("@angular/forms").FormGroup<{
            ...
            filters: import("@angular/forms").FormControl<AlarmRuleFilter[]>;
        }>;
    ...
    updateExpressionTypeValidator(type: AlarmRuleExpressionType | string): void;
