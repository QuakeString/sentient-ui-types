...
export interface GroupActionDescriptor<T extends BaseData<HasId>> {
    name: string;
    icon: string;
    isEnabled: boolean | ((entities: T[]) => boolean);
...
export type EntityTableColumnType = 'content' | 'action' | 'link' | 'entityChips' | 'sparkline';
...
/**
 * A cell rendered by the reusable `tb-spark-line` component: a tiny bar trend +
 * optional total. `dataFunction` returns the series (e.g. per-hour counts);
 * `totalFunction` (optional) the number shown beside it.
 */
export declare class SparkLineTableColumn<T extends BaseData<HasId>> extends BaseEntityTableColumn<T> {
    key: string;
    title: string;
    width: string;
    dataFunction: (entity: T) => number[];
    totalFunction: (entity: T) => number;
    cellStyleFunction: CellStyleFunction<T>;
    constructor(key: string, title: string, width?: string, dataFunction?: (entity: T) => number[], totalFunction?: (entity: T) => number, cellStyleFunction?: CellStyleFunction<T>);
}
export type EntityColumn<T extends BaseData<HasId>> = EntityTableColumn<T> | EntityActionTableColumn<T> | EntityLinkTableColumn<T> | EntityChipsEntityTableColumn<T> | SparkLineTableColumn<T>;
export declare class EntityTableConfig<T extends BaseData<HasId>, P extends PageLink = PageLink, L extends BaseData<HasId> = T> {
    ...
    /**
     * One-shot request to open the entity-details drawer on the tab with this
     * (translated) label — consumed and cleared by the details panel once its tabs
     * are ready. Matched by label (not index) so it survives tab reordering. Used
     * e.g. by an integration's "See debug events" action to land on the Events tab.
     */
    openEntityDetailsTabLabel: string;
    rowPointer: boolean;
    ...
    updateData(closeDetails?: boolean, silent?: boolean): void;
